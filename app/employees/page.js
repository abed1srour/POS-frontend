...
/**
 * Professional Employees page for the POS app (matches dashboard theme).
 * - Dark/light toggle (persisted in localStorage)
 * - Auth guard (redirects to /login if no token)
 * - Search, sort, filter, pagination
 * - Add / Edit / Delete employees (modals)
 * - Minimal CSV export of current page
 *
 * API endpoints expected (can be proxied through Next):
 *   GET    /api/employees?limit=&offset=&q=&sort=&order=
 *   POST   /api/employees
...
export default function EmployeesPage() {
  ...
  const [deletedEmployeesLoading, setDeletedEmployeesLoading] = useState(false);
  ...
  async function fetchList() {
    ...
    try {
      ...
      const res = await fetch(api(`/api/employees?${params.toString()}`), {
        headers: authHeaders(),
        cache: "no-store",
      });
  ...
  async function fetchDeletedEmployees() {
    setDeletedEmployeesLoading(true);
    try {
      const res = await fetch(api("/api/employees?includeDeleted=true&limit=100"), {
        headers: authHeaders(),
        cache: "no-store",
      });
      if (res.status === 401) return router.replace("/login");
      if (!res.ok) throw new Error(`Failed (${res.status})`);
      const data = await res.json();
      setDeletedEmployees(data.data || data || []);
    } catch (e) {
      console.error("Failed to fetch deleted employees:", e);
    } finally {
      setDeletedEmployeesLoading(false);
    }
  }
  ...
  function handleViewMore(employee) {
    router.push(`/employees/${employee.id}`);
  }

  function exportCSV() {
    const header = ["ID","First Name","Last Name","Phone","Address","Role","Status","Daily Pay","Hire Date","Created At"];
    ...
    a.href = url; a.download = `employees_page_${page}.csv`; a.click();
    URL.revokeObjectURL(url);
  }

  async function saveEmployee(input, isEdit) {
    const method = isEdit ? "PUT" : "POST";
    const path = isEdit ? `/api/employees/${input.id}` : "/api/employees";
    
    // Calculate hourly rate based on daily pay (assuming 8-hour workday)
    const hourlyRate = parseFloat(input.daily_pay) / 8;
  ...
  async function deleteEmployee(p) {
    const res = await fetch(api(`/api/employees/${p.id}`), { method: "DELETE", headers: authHeaders() });
    if (res.status === 401) return router.replace("/login");
  ...
  async function restoreEmployee(p) {
    const res = await fetch(api(`/api/employees/${p.id}/restore`), { method: "PATCH", headers: authHeaders() });
    if (res.status === 401) return router.replace("/login");
    if (!res.ok) throw new Error(`Restore failed (${res.status})`);
  }

  async function clearRecycleBin() {
    const res = await fetch(api(`/api/employees/recycle-bin/clear`), { method: "DELETE", headers: authHeaders() });
    if (res.status === 401) return router.replace("/login");
  ...
  function EmployeeForm({ initial, onCancel, onSaved }) {
    ...
    const hourlyRate = calculateHourlyRate(form.daily_pay);

    async function onSubmit(e) {
      e.preventDefault();
      setBusy(true); setError(null);
      try {
        await saveEmployee(form, Boolean(initial?.id));
        onSaved();
      } catch (e) {
    ...
    return (
      <form onSubmit={onSubmit} className="space-y-4">
        <h3 className="text-base font-semibold">{initial?.id ? "Edit employee" : "Add employee"}</h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Daily Pay ($)</label>
            <input 
              type="number"
              step="0.01"
              min="0"
              value={form.daily_pay} 
              onChange={(e)=>setForm({...form, daily_pay: e.target.value})} 
              required 
              className="w-full rounded-xl border border-gray-200 bg-white/70 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-white/5 dark:text-white" 
            />
          </div>
          <div>
            <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Role</label>
            <select 
              value={form.role} 
          ...
          <div>
            <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Hire Date</label>
            <input 
              type="date"
          ...
          <div>
            <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Hourly Rate</label>
            <div className="w-full rounded-xl border border-gray-200 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white">
              ${hourlyRate}/hour
            </div>
          </div>
        </div>
    ...
    <Layout title="Employees">
        ...
        <div className="overflow-hidden rounded-3xl border border-gray-200/60 bg-white/80 shadow-xl dark:border-white/10 dark:bg-white/5">
              ...
              <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-white/5 dark:to-white/10 text-sm text-gray-600 dark:text-gray-300">
                <tr>
                  {[
                    ...
                    { key: "phone", label: "Phone", w: "w-48" },
                    { key: "role", label: "Role", w: "w-32" },
                    { key: "status", label: "Status", w: "w-32" },
                    { key: "daily_pay", label: "Daily Pay", w: "w-32" },
                  ...
                  <tr key={String(r.id)} className="border-t border-gray-200/60 dark:border-white/10">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        r.role === 'admin' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
                        r.role === 'manager' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400' :
                        'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400'
                      }`}>
                        {r.role?.charAt(0).toUpperCase() + r.role?.slice(1) || 'Worker'}
                        </span>
                      </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        r.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                        r.status === 'inactive' ? 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400' :
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-sm text-gray-500 dark:text-gray-400">${r.daily_pay || '0.00'}</span>
                      </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                          ...
                          title="View employee details"
                           ...
                           <button
                          onClick={() => router.push(`/employees/${r.id}/time-tracking`)}
          ...
          {!loading && rows.length === 0 && (
            <div className="text-center py-12">
              <Icon.Badge className="h-16 w-16 text-gray-300 mx-auto mb-4" />
      ...
      <Modal open={Boolean(deleting)} onClose={()=>setDeleting(null)}>
        <div className="space-y-4">
          <h3 className="text-base font-semibold">Delete employee</h3>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button onClick={async()=>{ 
              if (!deleting) return; 
              const success = await deleteEmployee(deleting); 
              if (success) {
                setDeleting(null); 
                await fetchList(); 
              }
      ...
      <Modal open={recycleBinOpen} onClose={() => setRecycleBinOpen(false)}>
        <div className="space-y-4 max-h-[70vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-2">
              {deletedEmployees.length > 0 && (
                <button
                  ...
                  title="Clear all deleted employees"
                >
          ...
          <div className="flex-1 overflow-y-auto">
            {deletedEmployeesLoading ? (
              <div className="text-center py-8 text-gray-500">Loading deleted employees...</div>
            ) : deletedEmployees.length === 0 ? (
              <div className="text-center py-8 text-gray-500">No deleted employees found</div>
            ) : (
              <div className="space-y-2">
                {deletedEmployees.map((employee) => (
                  <div key={employee.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-200/60 bg-white/50 dark:border-white/10 dark:bg-white/5">
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{employee.first_name} {employee.last_name}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        ID: {employee.id} • Deleted: {new Date(employee.deleted_at).toLocaleDateString()}
                      </div>
                    </div>
                <button
                      onClick={async () => {
      ...
      <Modal open={clearBinConfirm} onClose={() => setClearBinConfirm(false)}>
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Are you sure you want to permanently delete all {deletedEmployees.length} items in the recycle bin? 
            This action cannot be undone and will permanently remove all deleted employees.
          </p>
          <div className="flex items-center justify-end gap-2">
            <button 
