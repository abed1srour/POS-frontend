...
export async function GET(request) {
  try {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/expenses?${backendParams.toString()}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(`Backend expenses returned ${response.status}`);
        return NextResponse.json({ error: 'Backend server error' }, { status: response.status });
      }
    } catch (fetchError) {
      console.log('Backend expenses connection failed:', fetchError.message);
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Backend expense creation returned ${response.status}:`, errorData);
      ...
      console.log('Backend expense creation connection failed:', fetchError.message);
      return NextResponse.json({ error: 'Backend server not available' }, { status: 503 });
    }
