...
export async function GET(request) {
  try {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/purchase-orders?${backendParams.toString()}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(`Backend purchase orders returned ${response.status}`);
        return NextResponse.json({ error: 'Backend server error' }, { status: response.status });
      }
    } catch (fetchError) {
      console.log('Backend purchase orders connection failed:', fetchError.message);
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Backend purchase order creation returned ${response.status}:`, errorData);
      ...
      console.log('Backend purchase order creation connection failed:', fetchError.message);
      return NextResponse.json({ error: 'Backend server not available' }, { status: 503 });
    }
