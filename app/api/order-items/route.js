  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Backend order-items returned ${response.status}`);
        return NextResponse.json({ error: 'Backend server error' }, { status: response.status });
      }
    } catch (fetchError) {
      console.log('Backend order-items connection failed:', fetchError.message);
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Backend order-item creation returned ${response.status}:`, errorData);
      ...
      console.log('Backend order-item creation connection failed:', fetchError.message);
      return NextResponse.json({ error: 'Backend server not available' }, { status: 503 });
    }
